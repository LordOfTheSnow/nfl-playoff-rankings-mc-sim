...
- Standalone HTML export footer now includes the export date and time: "Created by nfl-playoff-rankings-mc-sim v{version} on {YYYY-MM-DD HH:MM} UTC±HH:MM. — View on GitHub" (server-local time with numeric UTC offset, e.g. `2026-09-20 17:05 UTC+02:00`; resolved once per export, so every page of a bundle shows the same timestamp)
- Standalone HTML export (single page and every page of the ZIP bundle) now starts with the live app's header: a dark nav bar with the NFL logo and "NFL MONTE CARLO PLAYOFF SIM" brand (linking back to `index.html` in the bundle), plus the "independent project not affiliated with the NFL" disclaimer strip. `nfl.png` is now embedded/copied alongside the team and conference logos
