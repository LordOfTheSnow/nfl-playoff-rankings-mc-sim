...
- **`export.py`** — Renders the standalone HTML export (`POST /api/export/page`, `POST /api/export/bundle`): pure functions turning already-computed standings/statistics/schedule-grid/team/simulation data into "Modernist"-styled HTML, plus the ZIP bundle builder. `server.py`'s `_capture_json_response` reuses the existing `_handle_get_*` JSON handlers to get that data (by temporarily intercepting `_send_json_response`) rather than duplicating their computation.
